...
class ReservationService {
  ...
  async canReserveShift(staffId, shift, staffMember, userRole = 'staff') {
    try {
      // Check if shift is already reserved by someone else
      if (shift.status === 'reserved' && shift.reserved_by !== staffId) {
        return { canReserve: false, error: 'Tura este deja rezervată de alt membru al personalului' };
      }
  ...
  /**
   * Reserve a shift for a staff member
  ...
  async cancelReservation(shiftId, staffId, userRole = 'staff', username) {
    try {
      ...
      if (shift.status !== 'reserved') {
        return { success: false, error: 'Tura nu este rezervată' };
      }

      // Check permissions (own reservation or manager/admin)
      if (shift.reserved_by !== staffId && !['manager', 'admin'].includes(userRole)) {
        return { success: false, error: 'Poți anula doar propriile rezervări' };
      }
      ...
      if (updatedRows.length === 0) {
        return { success: false, error: 'Nu s-a putut anula rezervarea' };
      }
      ...
      return { success: false, error: 'Eroare la anularea rezervării' };
    }
  }

  /**
   * Get reservation statistics for a staff member
   * @param {number} staffId - Staff member ID
